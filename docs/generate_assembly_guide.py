...
def hardware_page(c: canvas.Canvas, page_no: int, print_manifest: dict) -> None:
    ...
    colors = [("M2", LIME, "NeoPixels, Pico 2, camera, servo horns"), ("M2.5", TEAL, "Head panels, LED carriers, ToF, bearing retainers"), ("M3", ORANGE, "Structure, motors, decks, shell, safety plates"), ("M4", RED, "Blue Sea fuse-block mounting")]
    x, y = 42, 500
    ...
    kit = [
        ...
        "M2/M2.5/M3 drivers, M4/M6 wrenches, hex keys, torque-conscious hand tools",
        ...
        "Washers, prevailing locknuts, metal spacers, cable strain relief",
        "Eye protection, wheel blocks, and a padded bench area",
    ]
    yy = 464
    for item in kit:
    ...
    rules = [
        ...
        "Use washers and locknuts on metal load paths. Printed threads are not acceptable for axles, motors, battery retention, or the E-stop.",
...
def build_pdf() -> None:
    ...
    instruction_page(c, page, "01", "Print and calibrate", "Coupon results set the large-part parameters.", ["codex_robot_body_v1_coupons.png", "codex_robot_body_v1_alignment_pilot.png"], "17 coupon parts; PETG and TPU production profiles; calipers; insert tool; sample M3/M2.5 hardware; 608 and 6807 bearings; Blue Sea 5045 retail fuse block.", ["Print coupons in their production material and profile. Let every part cool before measuring.", "Select insert, clearance, bearing-seat, journal, wall, lap, pilot, bumper, and fairing stations from the notched datum.", "Place the orange Blue Sea 5045 footprint gauge on the production deck, start both M4 paths, and verify its full body, cover-service, and single-side wire corridors clear adjacent hardware.", "Update shared CAD parameters and rerun the complete export/validator pipeline before printing large plates or buying the fuse block."], "Every selected station fits by hand as intended; the retail-block gauge seats cleanly; measurements are recorded; and the regenerated validator is green. The gauge is mechanical only, never an electrical test."); page += 1
    ...
    instruction_page(c, page, "04", "Build tray and battery cradle", "The retail-only tray has no integrated carry hardware.", ["codex_robot_body_v1_carry_interface.png", "codex_robot_body_v1_battery_interface.png"], "Split tray and underside seam plate; 117 x 92 mm BLF-1203AB cradle; four M3 screws; two 20 mm battery straps; nonconductive padding.", ["Join the tray halves with the printed underside seam plate and inspect both faces for full seating and cracks.", "Install the cradle and padding. Seat the BLF-1203AB flat at 110 x 75 x 27 mm; all four pads and side/end locators must meet without rocking or wrapper pinch.", "Route both 20 mm straps and the separate Powerpole discharge/DC charge leads through the rear corridor. Do not bend either lead at the wrapper exit.", "Move the unpowered robot with two hands under opposite tray edges. Never lift by the shell, head, bumper, wheels, battery straps, or wiring."], "Battery retention remains prototype-only until delivered lead fit, <=5.6 A sustained pack current, 45-minute runtime with 20% reserve, thermal/BMS behavior, and loaded retention tests pass."); page += 1
    ...
    instruction_page(c, page, "06", "Install front idlers", "Use the complete retail shoulder-bolt stack—no machined groove required.", ["codex_robot_body_v1_front_idler.png"], "2 pods; 4 x 608 bearings; 4 printed bearing rings; 2 WDS 615-M6-8-65 shoulder bolts; 2 M6 washers and prevailing locknuts; stock 14/19/3 mm spacers and 0.5 mm shims; 2 Pololu #2693 hubs; wheel and retainer screws.", ["Seat two cooled, coupon-approved 608 bearings in each pod. Install both flush printed outer-race rings with four M2.5 screws per pod.", "From the inboard bolt head outward, assemble the inner 608, 14 mm inner spacer, outer 608, 19 mm spacer, #2693 hub, 3 mm tuning spacer, and 0.5 mm shim on the 65 mm shoulder.", "Add the M6 washer and prevailing-torque locknut outboard. Tighten only against the metal shoulder stack; tune shims from measured hardware so neither bearing is preloaded.", "Tighten both hub set screws, fasten the PETG core with six M3 screws, seat tire and trim, bolt the pod to its tray pad, then spin by hand."], "The bolt head and nut are tool-accessible, both hub set screws engage, axial play is controlled without preload, and no plastic thread or friction fit retains the axle. Loaded skid-turn testing remains pending."); page += 1
    ...
    instruction_page(c, page, "08", "Mount controller and safety MCU", "Keep Linux control and deterministic stop hardware mechanically serviceable.", ["codex_robot_body_v1_motor_controller.png", "codex_robot_body_v1_safety_mcu.png"], "Controller plate; four 5 mm M3 metal spacers; Cytron MDDS10; four 6 mm M3 board standoffs; Pico 2 shelf; four shelf standoffs; four 6 mm M2 Pico standoffs.", ["Install the lower plate on four metal spacers above the tray.", "Mount the MDDS10 with terminals facing robot-front; preserve underside-pin, terminal, and cooling clearances.", "Install the separate safety shelf and Pico 2. Keep micro-USB, SWD, headers, watchdog, and NC-input wiring accessible."], "Polarity, terminal bend, cooling, watchdog, NC inputs, and motor-cut output are bench-tested with the motor branch fused and wheels lifted."); page += 1
    instruction_page(c, page, "09", "Build power deck and E-stop", "The E-stop de-energizes motor power independently of the Pi.", ["codex_robot_body_v1_power_deck.png", "codex_robot_body_v1_harness_routing.png", "codex_robot_body_v1_top_detail.png"], "145 x 116 mm deck; both Pololu regulators and seven M2 standoffs; Panasonic CB1A-R-M-12V relay with integral bracket; Blue Sea 5045 covered four-circuit ATO/ATC fuse block; two M4 mounting bolts; strain straps; IDEC XW1E.", ["Mount both regulators on metal standoffs. Keep the 5 V Pi and 6 V servo rails separate.", "Mount the Panasonic relay through its integral steel bracket to the dedicated deck boss. Keep terminal quick-connects insulated, restrained, and clear of the service envelope.", "Place the orange 5045 gauge first. If it clears, mount the purchased Blue Sea block through both M4 paths, label all four branches, and strap the wire exit. Lift the removable deck for power-off cover and fuse service.", "Route signal/audio/sensors left while fused switched power/motors stay right. Keep the battery-near feeder fuse and relay-cut motor branch outside the accessory block.", "Install the keyed E-stop panel, legend, purchased nut, and reinforced backing before deck access closes."], "Before power: purchased relay and fuse block fit; electrical review approves source/branch fuse values, terminals/wires/crimps, labels, selective short clearing, thermal behavior, relay drive/dropout, polarity, reset, and dual-NC cutoff.", RED); page += 1
